<!-- Tradução PT-BR do skill TIA Portal MCP -->

---
name: tiaportal-mcp
description: Drive Siemens TIA Portal (TIA Portal) end-to-end through the TiaMcpServer MCP plugin. Use whenever the user mentions TIA Portal, TIA Portal, STEP 7, WinCC, S7-1200/1500, PLC, HMI, SCL, LAD, STL, Openness, or asks to create/modify/compile/download a project. Always start by calling the `Bootstrap` tool — it returns environment status and the recommended next tool.
---
...
**Caminho mais curto dentro do pacote (ao ler somente arquivos do pacote)**  
...
## 0.1 Modelo fraco / iniciante: você só precisa destas 15 ferramentas (ignore as demais por enquanto)
...
| O que fazer | Use isto; não toque nos demais |
|---|---|
| Iniciar / verificar ambiente | `Bootstrap` → 然后照它返回的 `recommendedNextTool` 做 |
| Conectar ao TIA | `Connect`（新Projeto）或 `AttachToOpenProject`（已打开的Projeto） |
| Ver o conteúdo do projeto | `GetProjectTree`（拿real `softwarePath`，如 `PLC_1`），细看用 `GetSoftwareTree` / `GetBlocks` |
| **Criar um projeto completo** | `ScaffoldProject`（一次调用搞定 PLC+HMI，见 §0.5） |
...
| Validar compilação | `CompileAndDiagnosePlc`（Deve `errorCount=0`） |
| Salvar/finalizar | `SaveProject` → `Disconnect` |
| Erro de ambiente/não instala | **`Doctor`** — 一次性体检(TIA装没装/Openness组/连接状态)，每项给出修复办法；`fix=true`(默认)会自动把你加进 Openness 组(可能弹 UAC) |

**Regras rígidas (especialmente para modelos fracos):** ① 永远先 `Bootstrap`(或装不上时先 `Doctor`)，照
`recommendedNextTool` 走。② caminho只从 `GetProjectTree` 拿，绝不自己编。③ 写操作前先
`dryRun=true`。④ 收尾Deve `CompileAndDiagnosePlc`(0 错) + `SaveProject`。⑤ 拿不准
parâmetro名时，照本表/§8 的"精确parâmetro名"抄，Não猜。HMI 美化看 §12，库复用看 §15。

**Três recursos para reduzir a barreira de entrada:**
- **Lite 工具档位** — Iniciar server 时设环境变量 `TIA_MCP_PROFILE=lite`，`tools/list`
  只暴露 ~42 个 L0/L1 核心工具(而非全部 ~200)，弱模型不会在工具海里选错，VS Code 的
  128 工具上限也不再爆。默认仍是 full；要Completo工具就别设这个变量。一键写入宿主配置：
  `tia config --lite`。(v2.2.8 实测：full=201 工具含 L2，lite=43 工具无 L2。)
- **parâmetro容错** — `softwarePath` 现在容忍多余空格/大小写，单 PLC Projeto或唯一匹配时
  传"PLC"也能自动认到 `PLC_1`；找不到时报错会**列出可用 PLC caminho**。少数易错工具
  接受别名(`tableJson`↔`tagTableJson`、`screenJson`↔`designJson`、
  `blockJson`↔`fcBlockJson`、`name`↔`projectName`)。即便如此，**优先传准确valor**。
- **一键自检自修** — 见上表 `Doctor`。
...
```
templates/project-blueprints/scaffold_spec_start_stop.json   Controle de partida/parada (PLC+HMI)
templates/project-blueprints/scaffold_spec_motor.json        Controle de motor (启停闭锁 + 速度标定 + HMI)
```
...
{ "projectName": "MyProj",
  ...
  "hmiScreens": [ { "screenName": "Tela principal", "width": 800, "height": 480, "designJson": { /* §6.3 schema */ } } ],
...
### Como escolher o método de chamada (evite armadilhas)

| Cenário | Recomendação |
|------|------|
| **Cursor / Claude Desktop / VS Code（MCP stdio）** | `mcpServers.command` 指向包内 `TiaMcpServer.exe`，`args: []`。由客户端完成 MCP 握手，直接调 `tools/call`。 |
| **健康检查 / 是否已Iniciar** | `GET /mcp/health`（仅探活，不替代 MCP 协议） |
| **HTTP 自写脚本** | 需实现 **completo** MCP JSON-RPC 会话（如 `initialize`、部分Cenário下 SSE / `Mcp-Session-Id`），**Não**对 `POST /mcp` 只发单条裸 `tools/call` 就期望返回，Caso contrário易长时间阻塞。 |
...
When the user asks for a "demo / exemplo / example project", the minimal recipe above
...
```
skill/lad-cookbook/MCPVerify_FC_LAD.s7dcl    + .s7res  (FC: Série/Paralelo/SR/Comparação/Move/Add)
skill/lad-cookbook/MCPVerify_FB_LAD_v3.s7dcl + .s7res  (FB: Temporizador放 Static)
skill/lad-cookbook/MCPVerify_Mixed_LADSCL.s7dcl + .s7res  (FB: LAD+SCL 混编 — Intertravamento/Comparação+定时/SCLAritmética/Borda,
                                                          全部 import-verified 形式; 2026-06-11 Compilar0错)
```
...
(Normalmente fechado/negated contact, edges, timers, `Calc`…), **export a real block that uses them with
...
|---|---|---|
| 常开 / Normalmente fechado contact | `Contact( #x )` / `I_Contact( #x )` | — |
...
```
Contact          Contato normalmente aberto (add <Negated Name="operand"/> for Normalmente fechado)
Coil / SCoil / RCoil   线圈 / Set / Reset
O                Paralelo OR-box (TemplateValue Name="Card" = inputs count)
PBox / NBox      上升沿 / 下降沿
Gt / Lt / Eq / Ne / Ge / Le   Comparação (TemplateValue SrcType=Int|DInt|Real|Word|...)
Add / Sub / Mul / Div         Aritmética (SrcType + Card templates)
Move             Transferência (Card=1 normally)
TON / TOF / TP   IEC Temporizador (require <Instance Scope="LocalVariable|GlobalVariable" UId="…"><Component Name="..."/></Instance>; only inside FB or with explicit IDB)
...
<Wire UId="…">
  ...
  <NameCon UId="P2" Name="…"/> 多个 NameCon = Paralelo同时驱动多个 Part
</Wire>
...
1. **`UId` inside `<FlgNet>` MUST be decimal `xs:int`**, NOT hex. Block-level
   ...
   error: `UId 属性无效 - tipo …XMLSchema:int 的valor "2A" 无效`.
...
- `if/elsif` `condition` and `assignment` `source` accept a **single variable
  ...
  **The builder now hard-errors at build/`dryRun`** on such input (e.g.
  `SCL símbolo local inválido："RawMax <> RawMin"`) instead of silently emitting a
...
**Rule (F-CPU / PLC de segurança):** `TON` / `TOF` / `TP` **IEC timer instances** must
...
`07-Seleção de operação.xml`), **(3)** author in TIA and `ExportBlock`.
...
entries with **different** `UId`s (two `IdentCon`s) — see `07-Seleção de operação.xml` in a
...
```jsonc
{
  ...
  "items": [
    ...
    { "type": "Text", "name": "TitleText", "left": 24, "top": 20, "width": 600, "height": 36,
      "text": "Controle de motor · Motor Control", "font": { "Size": 22 },
    ...
    { "type": "Text", "name": "CardRunLabel", "left": 52, "top": 126, "width": 260, "height": 28,
      "text": "Estado de execução", "font": { "Size": 16 }, "properties": { "ForeColor": "0xFF334155", "BackColor": "0x00FFFFFF" } },
    ...
    { "type": "Text", "name": "CardSpeedLabel", "left": 384, "top": 126, "width": 260, "height": 28,
      "text": "Velocidade de rotação (rpm)", "font": { "Size": 16 }, "properties": { "ForeColor": "0xFF334155", "BackColor": "0x00FFFFFF" } },
    ...
    { "type": "Button", "name": "StartBtn", "left": 720, "top": 120, "width": 260, "height": 64,
      "text": "Iniciar START", "font": { "Size": 20 },
...
`Click` are NOT valid** in V21 and produce `System.ArgumentException: 未找到请求的valor`
...
3. `EnsureStartStopUnifiedHmi` — 会先调用 `EnsureUnifiedHmiConnection`，再用与
   `EnsureUnifiedHmiTag` **相同** 的规则写 **符号interconexão**（清掉Erro的 `%DB1…` 绝对地址），
   可选parâmetro：`plcName`、`connectionName`（默认 `HMI_Connection_1`）。HMI 标签表名
   默认 `默认tabela de variáveis`，PLC 符号需与 `StartPB`/`StopPB`/`EStop`/`RunOut` 一致。
...
```
tipo "Siemens.Engineering.Connection.ConnectionConfiguration" 的对象
无法转换为tipo "Siemens.Engineering.Connection.IConfiguration"
```
...
| Native Git / VCI | ✕ 不做(用文本Exportar替代 §16) | ✅ Git+CI | ✅ full Git UI |
